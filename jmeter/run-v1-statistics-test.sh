...
# V1 Statistics QPS Test Script
# This script runs a dedicated test for V1 Statistics API to measure maximum read QPS

set -e

echo "=========================================="
echo "V1 Statistics QPS Test"
echo "=========================================="
...
if [ -f "$SCRIPT_DIR/v1-statistics-test.jmx" ]; then
    # Running from jmeter directory
    TEST_PLAN="$SCRIPT_DIR/v1-statistics-test.jmx"
...
RESULTS_FILE="$RESULTS_DIR/v1-statistics-test.jtl"
REPORT_DIR="$RESULTS_DIR/v1-statistics-test-report"
...
echo "Application is running. Starting V1 Statistics test..."
echo ""
...
if [ -f "$RESULTS_FILE" ]; then
    echo ""
    echo "=========================================="
    echo "V1 Statistics Performance Analysis"
    echo "=========================================="
    ...
    if [ "$TOTAL_REQUESTS" -gt 0 ]; then
        ...
        # Calculate QPS from actual test data
        ...
        if (( $(echo "$QPS >= 2000" | bc -l) )); then
            echo "✓ High QPS: >= 2000 requests/second"
        elif (( $(echo "$QPS >= 1000" | bc -l) )); then
            echo "✓ Medium QPS: >= 1000 requests/second"
        else
            echo "⚠ Low QPS: < 1000 requests/second"
        fi
        
        # Generate test report
        REPORT_FILE="$RESULTS_DIR/v1-statistics-test-report.txt"
        cat > "$REPORT_FILE" << EOF
==========================================
V1 Statistics Test Report
==========================================
Test Date: $(date)
API Version: V1 (Traditional Thread)
Test Type: Statistics (Read QPS)
...
$(if (( $(echo "$QPS >= 2000" | bc -l) )); then echo "- QPS: High (>= 2000 qps)"; elif (( $(echo "$QPS >= 1000" | bc -l) )); then echo "- QPS: Medium (>= 1000 qps)"; else echo "- QPS: Low (< 1000 qps)"; fi)
...
echo ""
echo "V1 Statistics test completed successfully!" 